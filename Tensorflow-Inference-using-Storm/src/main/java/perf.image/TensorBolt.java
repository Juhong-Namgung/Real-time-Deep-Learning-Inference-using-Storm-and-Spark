...
//		Map pairs = (Map) input.getValue(0);
//		for(Object key : pairs.keySet()) {
//			String keyStr = new String((byte[]) key);
//			byte[] valuByte = ByteBuffer.wrap((byte[])pairs.get(keyStr)).array();
//
//			imageBytes = valuByte;
//		}

		imageBytes = (byte[])input.getValue(0);
		...
		try (Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {
			...
			collector.emit(new Values(imageBytes, labels.get(bestLabelIdx), labelProbabilities[bestLabelIdx] * 100f));
	...
	@Override	
	public void cleanup() {
		LOG.info("Total msg: " + count);
		
	}

	@Override
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("img", "label", "probability"));
	}
